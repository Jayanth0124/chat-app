...
import { 
  ...
  Search, Loader2, Check, UserMinus, MoreVertical, Hash, AtSign, Clock, Inbox
} from 'lucide-react';
...
export default function ManageFriends() {
    ...
    <div className="w-full md:w-[280px] bg-surface-container-lowest border-r border-outline-variant/30 flex flex-col h-full shrink-0">
      <div className="px-5 py-6 shrink-0 flex items-center justify-between">
        ...
        <button 
          ...
          className="p-1.5 rounded-full hover:bg-surface-container-high text-on-surface-variant transition-colors xl:hidden cursor-pointer"
        >
          <X size={18} />
        </button>
      </div>
  ...
  const renderDetailsPanel = () => {
    if (!selectedUser) {
      return (
        <div className="w-[320px] bg-surface-container-lowest border-l border-outline-variant/30 h-full hidden lg:flex flex-col shrink-0 relative overflow-y-auto">
          <button 
            onClick={() => setManageFriendsOpen(false)}
            className="absolute top-6 right-6 p-2 rounded-full hover:bg-surface-container-high text-on-surface-variant transition-colors cursor-pointer z-20"
          >
            <X size={20} />
          </button>
    ...
    // Determine relationship status for search results
    ...
    return (
      <div className="w-full md:w-[320px] bg-surface-container-lowest border-l border-outline-variant/30 h-full flex flex-col shrink-0 relative">
        ...
        {/* Mobile back button from details pane */}
        <button 
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-md flex items-center justify-center p-0 md:p-8 lg:p-12">
      <div className="w-full h-full max-w-7xl max-h-[900px] bg-surface md:rounded-3xl shadow-[0_0_80px_rgba(0,0,0,0.5)] border border-outline-variant/30 overflow-hidden flex relative animate-in zoom-in-95 duration-200">
        
        {/* Mobile slide architecture: Left/Main combined -> Details */}
        <div className={`w-full h-full flex flex-col md:hidden absolute inset-0 transition-transform duration-300 bg-surface ${selectedUser ? '-translate-x-full' : 'translate-x-0'}`}>
          {/* Mobile Header / Nav */}
          <div className="px-5 py-4 border-b border-outline-variant/30 flex items-center justify-between bg-surface-container-low shrink-0">
            <h2 className="text-lg font-black tracking-tight flex items-center gap-2">
              <Users size={20} className="text-primary" /> Orbit Network
            </h2>
            <button onClick={() => setManageFriendsOpen(false)} className="p-2 rounded-full hover:bg-surface-container-high transition-colors">
              <X size={20} />
            </button>
          </div>
          ...
          {/* Mobile List (reusing MainPanel layout but tailored for mobile view) */}
          <div className="flex-1 overflow-y-auto">
             {/* We strip out the outer div of main panel and just map the list */}
